...
export const AuthProvider = ({ children }) => {
  ...
  const [request, response, promptAsync] = Google.useAuthRequest({
    clientId:
      "972004153950-thdv01tk63uqtqqvffqei3qsbn0e18ci.apps.googleusercontent.com",
    iosClientId:
      "972004153950-mqse1tpvr0jao50reufk5jriogpsl2ja.apps.googleusercontent.com",
    androidClientId:
      "972004153950-uue4m48dvip9odnnlbm2cai268a6v9cv.apps.googleusercontent.com",
  });
  ...
  const login = useCallback(async (email, password, keepSignIn) => {
    ...
    try {
      ...
      if (keepSignIn) {
        ...
        else await SecureStore.setItemAsync(TOKEN_KEY, res.data.token);
      }
      return { success: true, user: res.data.userResult };
    } catch (e) {
      return { error: true, message: e.response?.data?.title };
  ...
  const register = async (email, name, password) => {
    ...
    try {
      const res = await instance.post(`api/auths/register`, {
        email,
        name,
        password,
      });
